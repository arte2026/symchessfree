...
function drop(ev) {
  ...
  if (
    ...
    destinationSquare.appendChild(piece);
	highlightMovedPiece(piece);   // 👈 ADD THIS
	if (!hasGameStarted) {
...
function checkForEndGame(){
	// 👇 NEW: Check for King of the Hill victory first
  if (checkKingOfTheHillWin()) {
    return; // Exit the function immediately if someone won
  }
  // 👆 ----------------------------------------------
...
function checkKingOfTheHillWin() {
  // The four winning center squares
  const centerSquares = ["d4", "d5", "e4", "e5"];

  // Check if any King is currently sitting on one of these squares
  for (let squareId of centerSquares) {
    let square = boardSquaresArray.find((s) => s.squareId === squareId);
    
    if (square && square.pieceType === "king") {
      let winner = square.pieceColor === "white" ? "White" : "Black";
      
      // Stop the game and announce the winner
      allowMovement = false;
      showAlert(getTranslation("alert_koth"));
      return true; // A win was detected
    }
  }
  return false; // No win detected yet
}
